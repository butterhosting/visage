...
describe(BotDetectionService.name, () => {
  ...
  ])("detects bots (crawlers): %s", async (userAgent) => {
    // when
    const isBot = await service.isBot({ userAgent });
    // then
  ...
  ])("detects bots (headless/automation): %s", async (userAgent) => {
    // when
    const isBot = await service.isBot({ userAgent });
    // then
    expect(isBot).toBeTrue();
  ...
  ])("detects allegedly real people: %s", async (userAgent) => {
    // when
    const isBot = await service.isBot({ userAgent });
    // then
